...
class AgentCredentialManager {
  constructor() {
    ...
    if (!process.env.JWT_SECRET) {
      console.log(
        '⚠️ JWT_SECRET not found in environment, using generated secret'
      );
   ...
   */
  createAgentToken(agentName, agentType, permissions, expiryDays = 90) {
  ...
  async cleanupExpiredTokens() {
    ...
    const validCredentials = credentials.filter(
      (cred) => new Date(cred.expiresAt) >= now
    );
 ...
 */
async function createPredefinedAgents() {
...
async function verifyToken(token) {
  ...
  if (result.valid && result.payload) {
    ...
    console.log(
      `   Permissions: ${result.payload.permissions?.join(', ') || 'None'}`
    );
    console.log(
  ...
  switch (command) {
    ...
    case 'verify':
      if (args[1]) {
        ...
        console.log(
          'Usage: node scripts/create-agent-credentials.js verify <token>'
        );
      }
      break;
    ...
    case 'revoke':
      if (args[1]) {
        ...
        console.log(
          'Usage: node scripts/create-agent-credentials.js revoke <agentId>'
        );
      }
      break;
