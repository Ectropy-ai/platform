...
locals {
  ...
  # S3 object key — override writes to bucket root, default uses {env}/{type}/{filename}
  object_key = var.object_key_override != null ? var.object_key_override : "${var.environment}/${var.config_type}/${
...
resource "aws_s3_object" "config_backup" {
  count = var.create_backup && var.object_key_override == null ? 1 : 0
