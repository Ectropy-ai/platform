/**
 * =============================================================================
 * ENTERPRISE NONCE-BASED CONTENT SECURITY POLICY (API GATEWAY)
 *
 ...
 *
 * OWASP COMPLIANCE: A03:2021 - Injection (XSS Protection)
 * DEPLOYMENT: Phase 1 Priority 1 - Security Hardening (2025-11-30)
 * =============================================================================
 */
...
function buildCSPHeader(directives: CSPDirectives, nonce: string): string {
  ...
  if (directives.defaultSrc.length) {
    parts.push(
      `default-src ${directives.defaultSrc.map(processValue).join(' ')}`
    );
  }
  if (directives.scriptSrc.length) {
    parts.push(
      `script-src ${directives.scriptSrc.map(processValue).join(' ')}`
    );
  }
...
export function cspNonceMiddleware() {
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (config.reportUri) {
      ...
      res.setHeader(
        headerName,
        `${currentHeader}; report-uri ${config.reportUri}`
      );
    }
...
export function cspReportHandler(req: Request, res: Response): void {
  try {
    ...
    // Log violation for analysis (using API Gateway logger)
