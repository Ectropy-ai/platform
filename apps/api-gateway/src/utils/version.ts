...
export function getVersion(): string {
  try {
    ...
    try {
      ...
      // Strategy 2: Navigate from __dirname (CommonJS)
      // Navigate up to monorepo root
      // Development: apps/api-gateway/src/utils -> root (4 levels)
      // Production: dist/apps/api-gateway/utils -> root (4 levels)
    ...
    return process.env['API_VERSION'] || '0.0.0';
  }
}

/**
 * Get the API Gateway package version (local package)
 * Use this only if independent package versioning is needed
 * @returns The API Gateway package version
 */
export function getLocalVersion(): string {
  try {
    // Navigate to local package.json: src/utils -> package root (CommonJS)
    const localPackageJsonPath = join(__dirname, '../../package.json');
    ...
    console.error(
      '[Version] Failed to read local version from package.json:',
      error
    );
    return '0.0.0';
  }
