...
function getEventType(method: string, statusCode: number): AuditEventType {
  // Security events
  if (statusCode === 401) {
    return AuditEventType.AUTHZ_PERMISSION_DENIED;
  }
  if (statusCode === 403) {
    return AuditEventType.AUTHZ_PERMISSION_DENIED;
  }
  if (statusCode === 429) {
    return AuditEventType.API_RATE_LIMIT_HIT;
  }
...
function extractResourceInfo(path: string): {
  ...
  const patterns = [
    ...
    { regex: /^\/api\/agents\/([^/]+)/, type: AuditResourceType.AGENT },
...
function determineSeverity(
  ...
  if (isSensitive && statusCode >= 200 && statusCode < 300) {
    return 'low';
  }
...
export const authAuditMiddleware = createAuditMiddleware({
  ...
  logRequestBody: false, // Never log auth request bodies
  logResponseBody: false,
});
