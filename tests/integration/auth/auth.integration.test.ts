...
describe('auth Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    service = new authService(testEnv.getConfig());
  });
  ...
  describe('Performance Under Load', () => {
    it('should handle high concurrency correctly', async () => {
      const operations = Array(50)
        .fill(null)
        .map((_, i) => service.create({ name: `Concurrent Test ${i}` }));
